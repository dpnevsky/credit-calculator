...
const rejectionReasonLabels: Record<string, string> = {
  ...
  UNEMPLOYED: 'Отсутствует подтвержденная занятость.',
...
const statusConfig: Record<string, { label: string; color: string }> = {
  DRAFT: { label: 'Предварительно одобрено', color: '#6b7280' },
  PRESCORING_REJECTED: { label: 'Отказано', color: '#dc2626' },
  SUBMITTED: { label: 'Отправлена', color: '#0ea5e9' },
  SCORING_COMPLETED: { label: 'Скоринг завершён', color: '#16a34a' },
  SCORING_APPROVED: { label: 'Одобрено', color: '#ca8a04' },
