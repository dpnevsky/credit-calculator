package com.dpnevsky.creditcalculator.scoring.infrastructure.persistence.repository;

import com.dpnevsky.creditcalculator.scoring.infrastructure.persistence.entity.ScoringResultEntity;
import org.springframework.data.jpa.repository.JpaRepository;

import java.util.List;
import java.util.UUID;

public interface ScoringResultRepository extends JpaRepository<ScoringResultEntity, UUID> {

    List<ScoringResultEntity> findByScoringRequestId(UUID scoringRequestId);
}